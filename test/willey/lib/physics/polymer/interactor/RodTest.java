...
public class RodTest extends AbstractTest
{
	...
	{
		Rod vRod = new Rod(of(1, 0, 0), of(0, 0, 0), 10, 0.5, 0, 0);
	...
	public void testTranslateOnly()
	{
		...
		Rod vTranslated = vRod.randomMove();
	...
	public void testBad()
	{
		Rod vRod = new Rod(of(0.0, 0.0, 10.0), of(0.9532062418932963,
				3.772754707565036, 0.0), 10, 0.5, 0, 0);
		Rod vNoRotation = vRod.randomMove();
	...
	public void testRotateOnly()
	{
		...
		Assert.assertTrue(vDir
				.coordinatesEqual(vRod.move(0.0, 0.0).direction()));

		Rod vNoRotation = vRod.move(0.0, 0.0);
		Assert.assertTrue(vNoRotation.position().coordinatesEqual(
				vRod.position()));
		Assert.assertTrue(vNoRotation.endPoint().coordinatesEqual(
				vRod.endPoint()));
	...
	{
		Rod vRod1 = new Rod(of(1, 0, 0), zeroVector(), 10, 0.5, 0, 0);
	...
	{
		Rod vRod1 = new Rod(of(1, 0, 0), zeroVector(), 10, 0.5, 0, 0);
		Rod vRod2 = new Rod(of(0,-1,0), of(1,1,0.7), 10, 0.5, 0, 0);
		Rod vRod3 = new Rod(of(1,0,0), zeroVector().add(randomUnitVector().scale(2)), 10, 0.5, 0, 0);
		Assert.assertTrue(vRod1.interacts(vRod2));
		Assert.assertTrue(vRod1.interacts(vRod2));
		Assert.assertFalse(vRod1.interacts(vRod3));
		for (int i=0; i < 1000; i++)
		{
			Rod vRod4 = vRod1.move(MathUtil.kRng.nextDouble(), 0.0);
			Assert.assertTrue(vRod1.interacts(vRod4));
		}
		
		for (int i=0; i < 1000; i++)
		{
			Rod vRod4 = vRod1.move(MathUtil.kRng.nextDouble() + 11, 0.0);
	...
	{
		return new Rod(randomUnitVector(), zeroVector(), 10, 0.5, pTranslation, pRotation);
	}
}
