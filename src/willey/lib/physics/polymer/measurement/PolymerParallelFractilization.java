...
public class PolymerParallelFractilization<PR extends PolymerAndRods> implements
		...
		for (int i=0; i < pMeasurable.getSize() - 1; i++)
		{
			for (int j=i; j < pMeasurable.getSize(); j++)
			{
				...
				} else
				{
					...
					vStepDistance.add(MeasurementUtil.parallelMagnitude(vMonomerPositions.get(i).subtract(
